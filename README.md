...
- 运动热量估算：主表盘根据步数估算并显示消耗热量
- 心率血氧功耗管理：主表盘低功耗采样，心率页正常采样，其他页面关断 MAX30102
- 磁力计功耗管理：仅指南针界面开启 QMC5883P，其他页面切换为挂起模式
- 环境传感器功耗管理：仅环境界面开启 BME280，其他页面切换为睡眠模式
- 六轴传感器功耗管理：首页低功耗计步，指南针界面正常 6 轴采样，其他页面关闭 LSM6DS3TR
...
- `calorie_calculator.*`：根据步数估算运动消耗热量，当前用于主表盘热量显示
- `heart_sensor_power.*`：心率血氧传感器功耗策略，按页面请求 MAX30102 低功耗、正常或关断模式
- `mag_sensor_power.*`：磁力计功耗策略，按指南针页面启停 QMC5883P
- `env_sensor_power.*`：环境传感器功耗策略，按环境页面启停 BME280
- `imu_sensor_power.*`：六轴传感器功耗策略，按首页计步和指南针页面切换 LSM6DS3TR 模式

### `src/drivers/`

- `BME280/`：温度、气压、湿度、海拔，支持正常测量和睡眠模式切换
- `LSM6DS3TR/`：加速度计与陀螺仪，支持关闭、低功耗计步和正常 6 轴采样模式
- `MAX30102/`：心率与血氧
- `QMC5883/`：磁力计，支持连续测量和挂起模式切换
- `RTC/`：实时时钟
...
1. 传感器线程初始化各传感器和 RTC
2. UI 定时器根据当前页面设置 `system_data` 中的采集开关和传感器功耗模式
3. 传感器线程按采集开关切换对应传感器模式、读取数据，并写入 `system_data`
4. UI 读取数据快照，主表盘根据步数估算热量并刷新控件
